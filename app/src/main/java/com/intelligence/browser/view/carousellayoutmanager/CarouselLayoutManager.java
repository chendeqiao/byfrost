...
public class CarouselLayoutManager extends RecyclerView.LayoutManager {
    private String TAG = "CarouselLayoutManager";
    ...
    protected int scrollBy(final int diff, @NonNull final RecyclerView.Recycler recycler, @NonNull final RecyclerView.State state) {
        ...
        if (mCircleLayout) {
            ...
            final int maxOffset = getMaxScrollOffset() - mDecoratedChildWidth / 6;//(mDecoratedChildWidth -mDecoratedChildPadding)/2;
    ...
    private void fillChildItem(final int start, final int top, final int end, final int bottom, @NonNull final LayoutOrder layoutOrder, @NonNull final RecyclerView.Recycler recycler, final int i, final boolean childMeasuringNeeded) {
        ...
        view.setTag(androidx.recyclerview.R.id.item_touch_helper_previous_elevation, i);
...
//        }
        ...
        if (itemPositionDiff < -2) {
            return 0.2625;
        } else if (itemPositionDiff < -1) {
            return absIemPositionDiff / 16 + 0.1375;
        } else if (itemPositionDiff < 0) {
            return absIemPositionDiff/5;//StrictMath.atan(absIemPositionDiff) / 5;
        } else {
            return absIemPositionDiff/2;
        }
    }
