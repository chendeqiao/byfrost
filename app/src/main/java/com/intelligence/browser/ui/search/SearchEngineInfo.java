...
public class SearchEngineInfo {
    ...
    private static final int FIELD_SEARCH_URI = 1;
    private static final int FIELD_ENCODING = 2;
    private static final int FIELD_SUGGEST_URI = 3;
    ...
    public SearchEngineInfo(SearchEngineEntity entity) {
        if (entity == null) {
            return;
        }
        mName = entity.getTitle();
        mSearchEngineData = new String[NUM_FIELDS];
        if (entity.getEngine_url() != null) {
            mSearchEngineData[FIELD_SEARCH_URI] = entity.getEngine_url();
        }
        // Default to UTF-8 if not specified.
        String enc = entity.getEncoding();
        if (TextUtils.isEmpty(enc)) {
            enc = "UTF-8";
        }
        mSearchEngineData[FIELD_ENCODING] = enc;
        mSearchEngineData[FIELD_LABEL] = mName;
    }
    /**
    ...
    private String getFormattedUri(String templateUri, String query) {
        ...
        try {
            if (templateUri.contains(PARAMETER_SEARCH_TERMS)) {
                ...
                return templateUri + URLEncoder.encode(query, enc);
            }
