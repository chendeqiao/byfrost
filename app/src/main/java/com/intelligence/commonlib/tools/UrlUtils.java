...
public class UrlUtils {
    ...
    public static final Pattern URL_PATTERN = Pattern.compile("^((https|http|ftp|rtsp|mms)?://)"
            + "+(([0-9a-z_!~*'().&=+$%-]+: )?[0-9a-z_!~*'().&=+$%-]+@)?"
            + "(([0-9]{1,3}\\.){3}[0-9]{1,3}"
            + "|"
            + "([0-9a-z_!~*'()-]+\\.)*"
    ...
    public static boolean isSearch(String inUrl) {
        ...
        return !Patterns.WEB_URL.matcher(urlLowcase).matches()
